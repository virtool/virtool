 ...
 *
 * Every state is present, and every workflow under each, with zeros written
 * rather than omitted, because a KEDA `metrics-api` trigger addresses one
 * figure by a fixed path (`pending.nuvs`) and a missing key is an error at the
 * scaler, not a zero.
 *
 ...
 *
 * **The three terminal states are always zero.** Grouping over the whole `jobs`
 * table is a scan that grows forever against a table this side cannot index, so
 * `readJobCounts` covers `pending` and `running` alone — see the bound recorded
 * on `readJobQueueBounded`. Nothing scales on how many jobs have already
 * finished, so the figures a scaler reads are the true ones; a caller wanting
 * lifetime totals is asking a question this endpoint has never been able to
 * afford.
 *
 * A `workflow` the union does not name is dropped rather than folded onto an
 * `other` key, because the shape has no such key and inventing one would put an
 * unannounced field in front of a scaler. The column is plain `text`, so this
 * is reachable; `/metrics` is where an unrecognised value is still counted.
 */
 ...
 *
 * Public, and it has to be: the scaler holds no job key, and could not — a key
 * is minted at claim time, which is the very thing it is deciding whether to
 * start a pod to do. What bounds the endpoint is the network, exactly as for
 * `POST /jobs/claim`: this service has no ingress.
 *
