...
The signed storage requests in the transfer step don't use the API key. The
...
3. Upload each piece with `PUT` to:
   ...
   Pieces can upload in parallel, but don't run more than `concurrency`
   requests at once.
...
These are standard HTTP requests and don't require an Azure SDK. If a request
