...
new instance and can be raised to 120,000,000,000 bytes (120 GB), the application
ceiling. A `size` above the configured maximum returns `413`; a size above
the application ceiling fails validation with `422`. Check the initialization response
...
| `413` | The declared `size` exceeds the configured maximum but is within the application ceiling. |
