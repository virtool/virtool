...
blob ceiling. A larger `size` is refused with a `413` before any bytes are
transferred, so check the response before starting the block uploads.
...
| `413` | The declared `size` is above the instance's maximum upload size. |
