...
new instance and can be raised to 120,000,000,000 bytes (120 GB), the app
ceiling. A `size` exceeding the configured max returns `413`; a size exceeding
the app ceiling fails validation with `422`. Check the initialization response
...
| `413` | The declared `size` exceeds the configured max but is within the app ceiling. |
