...
3. Tell Virtool to finalization the upload.
...
| `size` | integer | Exact file size in bytes, from 0 to the instance's max upload size. |

An administrator sets the max upload size, which is 5,000,000,000 bytes on a
new instance and can be raised to 120,000,000,000 bytes (120 GB), the application
ceiling. A `size` above the configured max returns `413`; a size above
...
| `413` | The declared `size` exceeds the configured max but is within the application ceiling. |
