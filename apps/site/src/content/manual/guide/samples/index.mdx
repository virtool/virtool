...
2. Choose the workflows (PathoscopeBowtie or Nuvs), subtraction, and references you
   ...
   Selecting multiple references starts a separate job for each sample-reference
   combination.
...
4. Populate the sample creation form.
   ...
   For paired data, check that the file orientation labels (left and right) are correct
   ...
   The default subtraction is preselected whenever you create an analysis for the sample.
   You can find the default subtraction for a given sample at
...
A median below 25 or a lower quartile below 10 is cause for concern. The sample shown
here has sufficient quality for further analysis.

### Nucleotide composition at read positions

![Nucleotide Composition at Read Positions](./nuc_comp_at_read_positions.png?url)

This plot shows the proportion of each nucleotide at every read position. In a
diverse library, nucleotide proportions shouldn't vary systematically by read
position, so the lines should be roughly parallel. Overall nucleotide
composition depends on the library; viral genomes may, for example, be
A/T-rich.
...
Each sample can be owned by a user group. This lets multiple groups of
diagnosticians or researchers keep their data private while sharing one Virtool
instance.
...
The sample owner or an administrator can change an existing sample's access
rights.
...
| _Primary Group_ | The sample is automatically assigned the creator's primary group                |
...
| _Read_         | Sample appears in searches. Users can view it but can't edit it or start analyses. |
| _Read & Write_ | Includes _Read_ rights. Users can also edit the sample and run analyses.                               |

#### All users' rights

This setting determines how any Virtool user can interact with the sample. These
rights behave like group rights and can be changed by sample owners and
