...
Sample FASTQ files are copied into a new sample directory. The files are compressed
...
Quick analyses allows you to start analysis jobs for many samples at once.
...
2. Choose the workflows (PathoscopeBowtie or Nuvs), subtraction, and references you
   want to use to analyze your samples.

   Selecting many references starts a separate job for each sample-reference
   combination.

3. Click <DummyButton icon="play">Start</DummyButton> to start the analysis jobs.
...
4. Populate the sample creation form.
   ...
   either _normal_, _sRNA_, or amplicon. Later analysis workflows run using
   ...
   The default subtraction for a sample is pre-selected as whenever you create an
   ...
   (**Banana**) is already pre-selected.
...
You see three different graphs on this page as shown below.
...
Median values of less than 25 or a lower quartile of less than 10 is about. For the preceding sample, the quality of our sample library is fine and further analysis can take place.
...
With all the things being equal in a diverse library, you should see an even distribution of the four bases which doesn't change with base position. Although the relative amount of G/C content is determined by your library, but what you should see on the graph are parallel lines going across the plot.

### Read-wise Quality Occurrence

![Read-wise Quality Occurrence](./read_wise_quality_occurrence.png?url)

In this case we are taking every sequence and looking at the mean score across all the bases in that particular sequence. The distribution of those means are then plotted as shown previously. All sequences should form one tight distribution (sharp curve) with universally high quality and no sequences of low quality. This sharp curve is the average quality per read. A mean quality below 27 is a cause for concern.
...
2. Click on the sample you would like to edit. Here we choose
...
Click on the sample you wish to delete. Here we delete **Test A**.
![Test A](./Test_A.png?url)

Click on the <Icon name="trash" color="red"/> to delete the sample. A dialog box such as the one below shows up to confirm the deletion of the sample.
![Test A Deletion](./test_A_deletion.png?url)

Click <Icon name="check" color="green" /> **Confirm**. The sample is now removed from the samples list.
...
A sample that has not yet been analyzed has no caches associated with it.
...
When you click on the link under **Cached Trims** you see all the parameters used by the trimming program as well as the name of the trimming command (`skewer-0.2.2`). The _hash_ is a unique identifier for the program-parameters combination used to trim this cache.
...
In the **Quality Distribution at Read Positions - Raw** chart we see that the mean base quality degrades as we get closer to the end of the read. This is a common issue in Illumina libraries. In the **Trimmed** charts we can see that low quality ends were removed resulting in a higher mean and min base quality.
...
Each sample can be owned by a specific user group. This allows many groups of diagnosticians or researchers to keep their data private or safe from one another while sharing a single Virtool instance.

Samples are not required to have an owner group. The group can be set to `None`. In this case, group rights settings have no effect.

#### Privileges

##### none

The management level (such as group or all users) cannot read or write the sample. Included users never see the sample in the sample management interface.
...
They cannot edit or remove the sample or create new analyses. The elements in the user interface associated with these actions are hidden.
...
Samples have their initial access rights configured when they are first created. How these rights are assigned can be configured in the administrative settings.
...
By default, sample names must be unique to the sample manager. This prevents confusion between identically named samples. It's possible to disable this feature. To do so, click **Settings** on the left sidebar under the samples overview page.
...
!["Unique Sample Names Off"](./unique_sample_names.png?url)
...
| _Read_         | Sample is returned in searches and is viewable. All editing interfaces are off and analyses cannot be started. |
| _Read & Write_ | Also to _Read_ rights, editing interfaces are enabled and analyses can be run.                               |
