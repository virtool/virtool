...
To use the Nuvs workflow to predict novel viruses, you must first install HMM
...
3. Wait for the installation to complete.
   ...
   When installation is complete, the HMM data loads. You can then browse the
...
2. Use the search bar to find HMMs by annotative name (for example, RNA replicase).

   <video src="./search.mp4" alt="Search HMMs for 'replicase'" autoplay />

3. Click an HMM item to view its details.
...
It's possible to delete all HMM data from the disk and database. This is used primarily
during testing and development.

You can reinstall the HMM data once the deletion process is complete.
...
3. Click <DummyButton color="red" icon="trash">Purge</DummyButton> to purge the HMM data.
   ...
   You are redirected to the HMM list view. The installation message is now
