...
For this example we focus on the five pathogens that have the greatest coverage.
...
The values for weight, depth, and coverage are all very high so we can be confident
...
Additionally when we take a look at the isolate coverage charts we see that the max
...
But, they do not exist in our reference database in Virtool because they were not
