...
/**
 * Start the spawner and runner loops — the `run` subcommand.
 *
 * One process carries both halves of Virtool's task system: the periodic
 * spawner, which inserts the scheduled tasks, and the runner, which claims and
 * executes them.
 *
 * The runner claims only the types `taskRegistry` names, so a row of any other
 * type is never claimed and stays queued.
 */
export async function startRun(): Promise<void> {
	try {
		...
		context.onShutdown("taskRunner", runner.stop, {
