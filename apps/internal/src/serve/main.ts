...
/**
 * Start the jobs API HTTP server — the `serve` subcommand.
 *
 * A function rather than module-scope side effects so the merged binary's
 * dispatcher decides when it runs, and so importing this module to read a type
 * opens no pool and binds no port.
 */
