 ...
 *
 * `run_fastqc` fills the one field and `finalize` sends it; it is null until
 * then rather than an empty blob, so a finalize reached without the quality
 * step having run fails instead of recording a sample with no quality data.
 */
