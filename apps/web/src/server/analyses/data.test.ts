...
describe("deleteAnalysis", () => {
	it("removes the row and returns the analysis as it was", async () => {
		...
		const deleted = await deleteAnalysis(db, storage, analysisId);
	...
	it("throws when the analysis does not exist", async () => {
		await expect(
			deleteAnalysis(db, new MemoryStorage(), 123456),
	...
	it("refuses an analysis that is still running", async () => {
		...
		await expect(
			deleteAnalysis(db, new MemoryStorage(), analysisId),
	...
	it("touches storage not at all for a Postgres-native analysis", async () => {
		...
		await deleteAnalysis(db, storage, analysisId);
