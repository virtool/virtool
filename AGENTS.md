...
- Schema changes that depend on the data being in a particular state are gated
  on a database operation — an audit or a resumable data migration recorded in
  `database_operations`. Declare the requirement in `MIGRATION_GATES`, register
  the implementation, and bump its version whenever it changes what it accepts.
  See [the operations guide](apps/internal/README.md#operations--audits-and-data-migrations).
