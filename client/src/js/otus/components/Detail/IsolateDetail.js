...
export class IsolateDetail extends React.Component {
  handleDownload = () => {
    followDownload(
      `/download/otus/${this.props.otuId}/isolates/${
        this.props.activeIsolate.id
      }`
    );
  };

  handleSetDefaultIsolate = () => {
    this.props.setIsolateAsDefault(
      this.props.otuId,
      this.props.activeIsolate.id
    );
  };
  ...
  render() {
    ...
    const defaultIsolateLabel = (
      <Label
        bsStyle="info"
        style={{ visibility: isolate.default ? "visible" : "hidden" }}
      >
    ...
    return (
      <div>
        ...
        <RemoveIsolate
          ...
          nextIsolateId={
            this.props.isolates.length ? this.props.isolates[0].id : null
          }
        />
