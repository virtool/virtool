...
describe("<Proxy />", () => {
  ...
  it("renders ProxyOptions correctly", () => {
    ...
    wrapper = shallow(
      <ProxyOptions {...props} enabled={false} proxyTestFailed={false} />
    );
    expect(wrapper).toMatchSnapshot();

    wrapper = shallow(
      <ProxyOptions {...props} enabled={true} proxyTestFailed={true} />
    );
    expect(wrapper).toMatchSnapshot();
  });
  ...
  describe("dispatch functions", () => {
    ...
    it("Submitting Username input dispatches updateSetting() action to update 'proxy_username' field", () => {
      ...
      expect(spyUpdateSetting.calledWith("proxy_username", "helloworld")).toBe(
        true
      );
    });
    ...
    it("Submitting Password input dispatches updateSetting() action to update 'proxy_password' field", () => {
      ...
      expect(spyUpdateSetting.calledWith("proxy_password", "123abc")).toBe(
        true
      );
    });
