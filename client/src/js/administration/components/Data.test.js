...
describe("<Describe />", () => {
    ...
    it("calls onSave callback function on input change and submission", () => {
        ...
        forEach(propsKeys, (value, index) => {
            ...
            wrapper.find("input").at(index).simulate("change", { target: { value: `${props[value]}0` } });
            wrapper.find("form").at(index).simulate(
                "submit",
                { preventDefault: jest.fn(), value: `${props[value]}0` }
            );

            if (value === "db_port") {
                expect(spySave.calledWith(propsKeys[index], Number(`${props[value]}0`))).toBe(true);
            } else {
                expect(spySave.calledWith(propsKeys[index], `${props[value]}0`)).toBe(true);
            }
        });
    });
    ...
    it("calls dispatch function onSave", () => {
        ...
        const adminElement = wrapper.children().at(0).shallow();
        const contentElement = adminElement.children().at(2).shallow();
        const divElement = contentElement.children().children().children().at(0)
            .shallow();

        divElement.children().at(0).prop("onSave")({ value: "foobar" });
