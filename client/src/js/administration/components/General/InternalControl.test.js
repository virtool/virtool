...
describe("<InternalControl />", () => {
  ...
  describe("dispatch actions", () => {
    ...
    it("selecting a search term results in internal control setting updating", () => {
      ...
      expect(spyUpdate.calledWith("123abc", { internal_control: "bar" })).toBe(
        true
      );

      wrapper.find(AsyncTypeahead).prop("onChange")([]);
      expect(spyUpdate.calledWith("123abc", { internal_control: "" })).toBe(
        true
      );
    });
    ...
    it("renders menu items from search results", () => {
      const result = wrapper
        .find(AsyncTypeahead)
        .prop("renderMenuItemChildren")({ id: "test", name: "example" });
