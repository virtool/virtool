...
export class SourceTypes extends React.Component {
  ...
  handleSubmit = e => {
    ...
    if (this.state.value !== "") {
      ...
      if (includes(this.props.sourceTypesArray, newSourceType)) {
        ...
        const newSourceTypes = this.props.sourceTypesArray.concat([
          newSourceType
        ]);
        this.props.onUpdate(
          newSourceTypes,
  ...
  render() {
    let isDisabled =
      (this.props.isGlobalSettings && this.props.isAdmin) ||
      this.props.restrictSourceTypes;
    ...
    const listComponents = map(
      this.props.sourceTypesArray.sort(),
      sourceType => (
        <SourceTypeItem
          key={sourceType}
    ...
    const errorMessage = (
      <div
        className={
          this.state.error ? "input-form-error" : "input-form-error-none"
        }
      >
...
const mapStateToProps = state => {
  ...
  const restrictSourceTypes = state.references.detail
    ? state.references.detail.restrict_source_types
    : null;
  const refId = state.references.detail ? state.references.detail.id : null;
  const isRemote = state.references.detail
