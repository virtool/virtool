...
const taskLimitKeys = flatMap(keys(taskDisplayNames), name => [
  `${name}_proc`,
  `${name}_mem`
]);

const taskSpecificLimitSelector = state =>
  pick(state.settings.data, taskLimitKeys);
...
export const maxResourcesSelector = createSelector(
  [resourcesSelector],
  resources => computeMaximums(resources)
);
...
export const checkTaskUpperLimits = createSelector(
  ...
  (resources, limits) => {
    ...
    const overMaxFields = pickBy(limits, (value, key) => {
      if (
        (endsWith(key, "proc") && maxProc < limits[key]) ||
        (endsWith(key, "mem") && maxMem < limits[key])
      ) {
        return true;
      }
      return false;
