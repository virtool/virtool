  ...
  render() {
    if (this.props.data.length) {
      let data = filter(
        this.props.data,
        otu =>
          this.props.filterOTUs
            ? otu.reads >= (otu.length * 0.8) / this.props.maxReadLength
            : true
      );
      ...
      const rows = map(data, (item, index) => {
        ...
        if (item.expanded) {
          const isolateComponents = map(
            sortBy(item.isolates, "pi").reverse(),
            isolate => {
          ...
          return components.concat(
            <div
              key={index}
              className="list-group-item pathoscope-otu-detail spaced"
            >
              {isolateComponents}
            </div>
          );
