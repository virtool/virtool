...
describe("<AnalysesList />", () => {
    ...
    describe("Dispatch Functions", () => {
        ...
        it("Entering search input dispatches filterAnalyses() action", () => {
            ...
            wrapper = shallow(<AnalysesListContainer store={store} />).dive();
            wrapper.find({ term: "search" }).prop("onFilter")({ target: { value: "example" } });
        ...
        it("Creating a new analysis dispatches analyze() action", () => {
            ...
            wrapper = shallow(<AnalysesListContainer store={store} />).dive();
            wrapper.find({ userId: "test-user" })
                .prop("onSubmit")(sampleId, references, algorithm, userId);

            expect(spy.calledWith(sampleId, references[0].refId, algorithm, userId)).toBe(true);
