...
describe("<Analyses />", () => {
    ...
    it("renders correctly", () => {
        wrapper = shallow(
            <AnalysesContainer store={store} analyses={null} match={{ params: { sampleId: "hello" } }} />
        ).dive();
        expect(wrapper).toMatchSnapshot();

        wrapper = shallow(
            <AnalysesContainer store={store} analyses={[]} match={{ params: { sampleId: "world" } }} />
        ).dive();
        expect(wrapper).toMatchSnapshot();
    });
    ...
    it("Component on mount dispatches findAnalyses() action to fetch analyses list", () => {
        ...
        wrapper = mount(
            <MemoryRouter initialEntries={["/samples/test-sample"]}>
                <Route
                    component={({ match }) => <AnalysesContainer store={store} match={match} />}
