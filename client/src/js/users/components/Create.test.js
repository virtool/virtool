...
describe("<CreateUser />", () => {
    ...
    describe("Dispatch Functions", () => {
        ...
        it("Submitting form dispatches createUser() action", () => {
            ...
            wrapper.find({ label: "Username" }).at(0)
                .prop("onChange")({ target: { name: "userId", value: "test-user" } });
            wrapper.find({ label: "Password" }).at(0)
                .prop("onChange")({ target: { name: "password", value: "test-password" } });
            wrapper.find({ label: "Confirm" }).at(0)
                .prop("onChange")({ target: { name: "confirm", value: "test-password" } });
