...
class Groups extends React.Component {
    ...
    if (prevState.groups.length < nextProps.groups.length) {
      return {
        activeId: prevState.createGroupId
          ? toLower(prevState.createGroupId)
          : prevState.activeId,
        createGroupId: "",
        groups: nextProps.groups
      };
  ...
  render() {
    ...
    if (activeGroup) {
      members = filter(this.props.users.documents, user =>
        includes(user.groups, activeGroup.id)
      );
    }
        ...
        (result, value, key) => {
          result.push(
            <ListGroupItem
              key={key}
              onClick={() =>
                this.props.onSetPermission(activeGroup.id, key, !value)
              }
            >
              <code>{key}</code>{" "}
              <Icon
