...
export const getTerm = state => get(state, "router.");
...
export function* createSample(action) {
  ...
  yield setPending(
    apiCall(samplesAPI.create, action, CREATE_SAMPLE, {}, extraFunc)
  );
}
...
export function* updateSample(action) {
  ...
  yield setPending(
    apiCall(samplesAPI.update, action, UPDATE_SAMPLE, {}, extraFunc)
  );
}

export function* updateSampleRights(action) {
  yield setPending(
    apiCall(samplesAPI.updateRights, action, UPDATE_SAMPLE_RIGHTS)
  );
}
