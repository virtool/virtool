...
class SampleRights extends React.Component {
  ...
  render() {
    ...
    const groupRights =
      (this.props.group_read ? "r" : "") + (this.props.group_write ? "w" : "");
    const allRights =
      (this.props.all_read ? "r" : "") + (this.props.all_write ? "w" : "");
    ...
    return (
      <div>
        <Alert bsStyle="info" icon="info-circle">
          Restrict who can read and write this sample and which user group owns
          the sample.
        </Alert>
