...
const getReadyHosts = props =>
  props.readyHosts && props.readyHosts.length
    ? props.readyHosts[0].id || ""
    : "";
...
class CreateSample extends React.Component {
  ...
  autofill = () => {
    this.setState({
      name: split(
        replace(this.state.selected[0], /[0-9a-z]{8}-/, ""),
        /_S\d+/
      )[0]
    });
  };
  ...
  render() {
    ...
    const libraryType =
      this.state.selected.length === 2 ? "Paired" : "Unpaired";
            ...
            <Row>
              <Col xs={12}>
                ...
                <InputGroup>
                  ...
                  />
                  <InputGroup.Button
                    style={{ verticalAlign: "top", zIndex: "0" }}
                  >
                    <Button
                      type="button"
...
const mapDispatchToProps = dispatch => ({
  ...
  onCreate: ({ name, isolate, host, locale, srna, subtraction, files }) => {
    dispatch(
      createSample(name, isolate, host, locale, srna, subtraction, files)
    );
  },
