...
export const list = ({ page }) => Request.get(`/api/refs?page=${page}`);

export const filter = ({ term }) => Request.get(`/api/refs?find=${term}`);
...
export const edit = ({ refId, update }) =>
  Request.patch(`/api/refs/${refId}`).send(update);
...
export const updateRemote = ({ refId }) =>
  Request.post(`/api/refs/${refId}/updates`).send({});
