...
describe("<ReferenceItem />", () => {
    ...
    it("renders base component correctly", () => {
        ...
        wrapper = shallow(<ReferenceItem {...props} />);
        expect(wrapper).toMatchSnapshot();

        props = {...props, processes: [{ id: "123abc", progress: 0.5, step: "test" }]};
        wrapper = shallow(<ReferenceItem {...props} />);
        expect(wrapper).toMatchSnapshot();

        props = {...props, processes: [{ id: "345def", progress: 1, step: "test" }]};
        wrapper = shallow(<ReferenceItem {...props} />);
        expect(wrapper).toMatchSnapshot();
    });
    ...
    it("renders <ReferenceMetadata /> subcomponent correctly", () => {
        ...
        wrapper.setProps({
            origin: { method: "Cloned from", data: { id: "123clone", name: "test-clone" } },
            organism: "",
            progress: 0
        });
        expect(wrapper).toMatchSnapshot();
    ...
    it("Clicking on reference card dispatches router location change", () => {
        ...
        wrapper.children().children().children().prop("onClick")(mockEvent);
        expect(spy.called).toBe(false);
