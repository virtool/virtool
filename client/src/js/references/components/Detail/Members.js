...
const getOtherMembers = (list, members) => {
  const otherMembers = filter(
    list,
    member => !some(members, ["id", member.id])
  );
  return otherMembers;
};
...
class ReferenceMembers extends React.Component {
  ...
  render() {
    ...
    const list =
      this.props.noun === "users" ? this.props.userList : this.props.groupList;
    const currentMembers =
      this.props.noun === "users" ? this.props.users : this.props.groups;
