...
export const ReferenceHeader = ({ name, createdAt, user, refId }) => (
  <div style={{ marginLeft: "5px" }}>
    <Row>
      <strong>{name}</strong>
      <Link
        to={{ state: { newReference: true, cloneReference: true, refId } }}
        style={{ float: "right" }}
      >
...
export const ReferenceMetadata = ({
  ...
  if (origin.method === "Imported from") {
    ...
    originData = (
      <a
        href={`https://www.github.com/${origin.data.slug}`}
          ...
          <tr>
            ...
            <td>
              ...
              <div className="text-muted" style={{ fontSize: "12px" }}>
                Created <RelativeTime time={latest_build.created_at} /> by{" "}
                {latest_build.user.id}
              </div>
            </td>
          </tr>
        </React.Fragment>
