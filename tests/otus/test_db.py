...
async def test_create_otu(
    ...
    if otu_id:
        await create_otu(app, "foo", "Bar", abbreviation, "bob", otu_id)
    else:
        await create_otu(app, "foo", "Bar", abbreviation, "bob")
...
async def test_edit(
    ...
    await edit(app, "6116cba1", "Foo Virus", abbreviation, None, "bob")
...
async def test_generate_otu_fasta(dbi, test_otu, test_sequence):
    ...
    await dbi.sequences.insert_many(
        [
            test_sequence,
            dict(
                test_sequence, _id="AX12345", sequence="ATAGAGGAGTTA", isolate_id="baz"
            ),
        ]
    )
