...
async def test_get_current_id_and_version(exists, has_ref, test_indexes, dbi):
    ...
    index_id, index_version = await virtool.db.indexes.get_current_id_and_version(dbi, ref_id)
...
async def test_get_next_version(empty, has_ref, test_indexes, dbi):
    ...
    assert await virtool.db.indexes.get_next_version(dbi, "hxn167" if has_ref else "foobar") == expected
...
async def test_tag_unbuilt_changes(dbi, create_mock_history):
    ...
    await virtool.db.indexes.tag_unbuilt_changes(dbi, "hxn167", "foo", 5)
