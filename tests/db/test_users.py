...
async def test_compose_groups_update(groups, test_dbi, kings, all_permissions, no_permissions):
    await test_dbi.groups.insert_many([kings])

    coroutine = virtool.db.users.compose_groups_update(test_dbi, groups)
...
async def test_compose_primary_group_update(primary_group, test_dbi, bob, kings, peasants):
    await test_dbi.users.insert_one(bob)

    await test_dbi.groups.insert_many([kings, peasants])

    coroutine = virtool.db.users.compose_primary_group_update(test_dbi, "bob", primary_group)
...
@pytest.mark.parametrize("force_reset", [None, True, False])
async def test_create(exists, force_reset, mocker, test_dbi, bob):
    ...
    if force_reset is None:
        args = (test_dbi, user_id, password)
    else:
        args = (test_dbi, user_id, password, force_reset)
    ...
    # Ensure the new user document is created and returned if the user_id is valid.
        ...
        assert await test_dbi.users.find_one() == document == bob
...
async def test_validate_credentials(legacy, user_id, password, result, test_dbi):
    """
    ...
    await test_dbi.users.insert_one(document)

    assert await virtool.db.users.validate_credentials(test_dbi, user_id, password) is result


@pytest.mark.parametrize("administrator", [True, False])
@pytest.mark.parametrize("elevate", [True, False])
@pytest.mark.parametrize("missing", [True, False])
async def test_update_sessions_and_keys(administrator, elevate, missing, test_dbi, all_permissions, no_permissions):
    """
    ...
    await test_dbi.keys.insert_one({
        "_id": "foobar",
        "administrator": False,
        "groups": ["peasants"],
    ...
    await test_dbi.sessions.insert_one({
        "_id": "foobar",
        "administrator": False,
        "groups": ["peasants"],
        "permissions": permissions,
    ...
    assert await test_dbi.sessions.find_one() == {
        "_id": "foobar",
        "administrator": administrator,
        "groups": ["peasants", "kings"],
    ...
    assert await test_dbi.keys.find_one() == {
        "_id": "foobar",
        "administrator": administrator,
        "groups": ["peasants", "kings"],
        "permissions": permissions,
