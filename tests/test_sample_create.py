...
@pytest.fixture
def test_create_sample_job(tmpdir, loop, test_motor, test_dispatch):
    ...
    job = virtool.sample_create.CreateSample(
        loop,
        executor,
        test_motor,
        settings,
...
def test_force_makedirs(exists, tmpdir):
    ...
    virtool.sample_create.force_makedirs(sample_path)
...
def test_move_trimming_results(paired, tmpdir):
    ...
    virtool.sample_create.move_trimming_results(path, paired=paired)
