...
the contents go, the directory stays — and Vitest runs test files in parallel
processes, so a shared path means one test deleting another's tree mid-run,
which surfaces as a missing file in whichever test lost the race. `mkdtemp` guarantees uniqueness per
