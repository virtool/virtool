...
export function parseHmmerTblout(lines: Iterable<string>): HmmerHit[] {
	...
	for (const line of lines) {
		...
		const [sequenceIndex, orfIndex] = fields[2]
			.split("_")[1]
			.split(".")
		...
		hits.push({
			...
			best_bias: Number.parseFloat(fields[8]),
			best_e: Number.parseFloat(fields[7]),
			best_score: Number.parseFloat(fields[9]),
			cluster,
			full_bias: Number.parseFloat(fields[6]),
			full_e: Number.parseFloat(fields[4]),
			full_score: Number.parseFloat(fields[5]),
			orfIndex,
			sequenceIndex,
		});
	}
