...
`apps/internal/src/serve/app.ts` references the other, but both record an
ordinary `Error` that escapes request handling as `status="500"`. This is the
status their own framework's unhandled-error path actually answers with
(h3's `toResponse` for web and Hono's `onError` for the jobs API). They
...
server-function name versus a Hono route's registered pattern. Making them
identical would mean inventing a route-pattern label for
