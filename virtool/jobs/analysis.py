...
async def new(db, settings, manager, sample_id, user_id, algorithm):
    ...
    """
    # Get the current id and version of the species index currently being used for analysis.
    index_id, index_version = await virtool.db.indexes.get_current_index(db)
    ...
    if "pathoscope" in algorithm:
        sequence_species_map = dict()
        species_dict = dict()

        async for sequence_document in db.sequences.find({}, ["species_id", "isolate_id"]):
            species_id = sequence_document["species_id"]

            species = species_dict.get(species_id, None)

            if species is None:
                species = await db.species.find_one(species_id, ["last_indexed_version"])

                try:
                    last_index_version = species["last_indexed_version"]

                    species_dict[species["_id"]] = {
                        "id": species["_id"],
                        "version": last_index_version
                    }

                    sequence_species_map[sequence_document["_id"]] = species_id
                except KeyError:
                    species_dict[species["id"]] = False

            sequence_species_map[sequence_document["_id"]] = species_id

        sequence_species_map = [item for item in sequence_species_map.items()]
...
async def format_analysis(db, analysis):
    if "pathoscope" in analysis["algorithm"]:
        ...
        for hit in analysis["diagnosis"]:
            ...
            version = hit["species"]["version"]

            species = formatted.get(species_id, None)

            if species is None:
                # Get the species entry (patched to correct version).
                _, species_document, _ = await virtool.db.history.patch_to_version(
                    db,
                    species_id,
                    version
                )
                ...
                for isolate in species_document["isolates"]:
                    max_ref_length = max(max_ref_length, max([len(s["sequence"]) for s in isolate["sequences"]]))

                species = {
                    "id": species_id,
                    "name": species_document["name"],
                    "version": species_document["version"],
                    "abbreviation": species_document["abbreviation"],
                    "isolates": species_document["isolates"],
                    "length": max_ref_length
                }

                formatted[species_id] = species
            ...
            for isolate in species["isolates"]:
                for sequence in isolate["sequences"]:
                    if sequence["_id"] == hit["id"]:
                        ...
                        del sequence["species"]
                        del sequence["species_id"]
                        del sequence["isolate_id"]

        analysis["diagnosis"] = [formatted[species_id] for species_id in formatted]

        for species in analysis["diagnosis"]:
            for isolate in list(species["isolates"]):
                if not any((key in sequence for sequence in isolate["sequences"]) for key in ("pi", "final")):
                    species["isolates"].remove(isolate)
                    continue
...
class Pathoscope(Base):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.sequence_species_map = {item[0]: item[1] for item in self.task_args["sequence_species_map"]}

        self.species_dict = self.task_args["species_dict"]

    @virtool.jobs.job.stage_method
    async def generate_isolate_fasta(self):
        """
        Identifies species hits from the initial default species mapping.

        """
        fasta_path = os.path.join(self.analysis_path, "isolate_index.fa")

        sequence_ids = list(self.intermediate["to_species"])

        ref_lengths = dict()

        # Get the database documents for the sequences
        async with aiofiles.open(fasta_path, "w") as handle:
            # Iterate through each species id referenced by the hit sequence ids.
            for species_id in await self.db.sequences.distinct("species_id", {"_id": {"$in": sequence_ids}}):
                # Write all of the sequences for each species to a FASTA file.
                async for document in self.db.sequences.find({"species_id": species_id}, ["sequence"]):
                    await handle.write(">{}\n{}\n".format(document["_id"], document["sequence"]))
                    ref_lengths[document["_id"]] = len(document["sequence"])

        del self.intermediate["to_species"]
    ...
    async def pathoscope(self):
        ...
        for ref_id, hit in report.items():
            # Get the species info for the sequence id.
            species = self.species_dict[self.sequence_species_map[ref_id]]

            # Raise exception if species is ``False`` (meaning the species had no ``last_indexed_version`` field).
            if not species:
                raise ValueError("Document has no last_indexed_version field.")

            hit["id"] = ref_id

            # Attach "species" (id, version) to the hit.
            hit["species"] = species
    ...
    async def import_results(self):
        ...
        any species indexes that may become unused when this analysis completes.
...
class PathoscopeBowtie(Pathoscope):
    ...
    @virtool.jobs.job.stage_method
    async def map_species(self):
        """
        Using ``bowtie2``, maps reads to the main species reference. This mapping is used to identify candidate species.
...
class NuVs(Base):
    ...
    1. Eliminate known viral reads by mapping the sample reads to the Virtool species reference using ``bowtie2`` saving
       unaligned reads.
    ...
    async def map_species(self):
        """
        Maps reads to the main species reference using ``bowtie2``. Bowtie2 is set to use the search parameter
        ``--very-fast-local`` and retain unaligned reads to the FASTA file ``unmapped_species.fq``.
        ...
        command = [
            ...
            "--very-fast-local",
            "-x", self.index_path,
            "--un", os.path.join(self.analysis_path, "unmapped_species.fq"),
            "-U", ",".join(self.read_paths)
        ]
    ...
    async def map_subtraction(self):
        """
        Maps unaligned reads from :meth:`.map_species` to the sample's subtraction host using ``bowtie2``. Bowtie2 is
        ...
        command = [
            ...
            "-U", os.path.join(self.analysis_path, "unmapped_species.fq"),
        ]
    ...
    async def import_results(self):
        ...
        After the import is complete, :meth:`.indexes.Collection.cleanup_index_files` is called to remove any species
