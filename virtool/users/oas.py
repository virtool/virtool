...
class EditUserSchema(BaseModel):
    administrator: Optional[bool] = Field(description="set the user’s administrator status")
    force_reset: Optional[bool] = Field(description="force a password reset next time the user logs in")
    groups: Optional[List[str]] = Field(description="the ids of the groups the user belongs to")
    password: Optional[str] = Field(description="the new password")
    primary_group: Optional[str] = Field(description="the users primary group used for sample rights")
