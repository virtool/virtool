        ...
        for _ in range(0, retries):
            try:
                while True:
                    ...
                    blast_coroutine = self.data.blast.check_nuvs_blast(
                        self.analysis_id, self.sequence_index
                    )

                    blast = await asyncio.wait_for(blast_coroutine, 600)
                ...
                logger.info("BLAST timed out: %s", self.rid)
                ...
                logger.info("Blast failed due to error: %s", err)
