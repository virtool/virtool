...
class Session:
    ...
    def __init__(self, session_document):
        ...
        self.authorized = session_document["authorized"]

        self.user_id = session_document.get("user_id")
        self.groups = session_document.get("groups")
        self.permissions = session_document.get("permissions")
...
async def middleware_factory(app, handler):
    async def middleware_handler(request):
        ...
        if session_id:
            document = await app["db"].sessions.find_one({"_id": session_id})
