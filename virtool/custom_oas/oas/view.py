...
from virtool.custom_oas.oas.struct1 import OpenApiSpec3, OperationObject, PathItem

from virtool.custom_oas.oas import docstring_parser


from virtool.custom_oas.injectors import _parse_func_signature

from virtool.custom_oas.utils import is_pydantic_base_model

from virtool.custom_oas.view import PydanticView, is_pydantic_view

from virtool.custom_oas.oas.typing1 import is_status_code_type
...
class _OASResponseBuilder:
    ...
    def _handle_pydantic_base_model(self, obj, example):
        if is_pydantic_base_model(obj):
            ...
            try:
                test = response_schema["example"]
                example.append(test)
                response_schema.pop("example", None)
            except KeyError:
                pass
            return response_schema
        return {}

    def _handle_list(self, obj, example):
        if typing.get_origin(obj) is list:
            return {
                "type": "array",
                "items": self._handle_pydantic_base_model(typing.get_args(obj)[0], example),
            }
        return self._handle_pydantic_base_model(obj, example)
    ...
    def _handle_status_code_type(self, obj):
        if is_status_code_type(typing.get_origin(obj)):
            ...
            example = []
            test = self._handle_list(typing.get_args(obj)[0], example)
...
def _add_http_method_to_oas(
    ...
    if body_args:
        ...
        try:
            example = body_schema["example"]
            body_schema.pop("example", None)
        except KeyError:
            pass

        if example:
            oas_operation.request_body.content = {
                "application/json": {"schema": body_schema, "example": example}
            }
        else:
            oas_operation.request_body.content = {
