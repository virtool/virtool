...
class PathoscopeSNAP(Pathoscope):
    ...
    def map_viruses(self):
        ...
        # Run the SNAP command append all STDOUT to a list for use in later stages.
        self.intermediate["to_viruses_count"] = 0
        self.intermediate["last_reported_progress"] = 0
        self.intermediate["to_viruses"] = list()

        self.run_process(command, stdout_handler=self.intermediate["to_viruses"].append)
    ...
    def save_mapped_reads(self):
        mapped_read_ids = [line["data"].split()[0] for line in self.intermediate["to_isolates"]]

        mapped_read_ids = list(set(mapped_read_ids))
        ...
        with open(os.path.join(self.paths["analysis"], "mapped.fastq"), "w") as mapped_file:
            for file in files:
                for record in SeqIO.parse(file, "fastq"):
                    if record.id in mapped_read_ids:
                        SeqIO.write(record, mapped_file, "fastq")
