...
class PathoscopeBowtie(Pathoscope):
    ...
    def map_viruses(self):
        ...
        to_viruses = virtool.pathoscope.sam.Minimal()
    ...
    def map_isolates(self):
        if self.intermediate["use_isolates"]:
            ...
            self.intermediate["to_isolates"] = self.run_process(command, no_output_failure=True)

            with open(self.paths["analysis"] + "/isolates.sam", "w") as isolate_sam_file:
                for line in self.intermediate["to_isolates"]:
                    isolate_sam_file.write(line + "\n")
