...
class PermissionRoutePolicy(DefaultRoutePolicy):
    def __init__(self, permission: LegacyPermission) -> None:
    ...
    async def check(
        ...
        Raises ``APIForbidden`` if the client does not have the required permission.
        ...
        * The user has the required permission in their legacy MongoDB-based
        ...
        if not (client.administrator_role or client.permissions[self.permission.value]):
            msg = f"Requires permission '{self.permission.value}' or administrator role"
            raise APIForbidden(msg)
...
def get_handler_flag(handler: Callable, method: str, name: str, default: Any):
    ...
    if isclass(h) and issubclass(h, APIView):
